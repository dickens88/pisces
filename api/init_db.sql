...
CREATE TABLE `t_incidents` (
  ...
  `close_reason` enum('False detection','Resolved','Repeated','Other') DEFAULT NULL COMMENT '关闭原因',
  ...
  `alert_list` text COMMENT '关联告警ID列表（JSON格式）',
  `graph_data` longtext COMMENT '事件图谱数据（JSON字符串）',
  `graph_summary` text COMMENT '事件图谱摘要',
  PRIMARY KEY (`id`),
